...
class PositionPrediction(nn.Module):
    def __init__(self, input_dim):
        ...
        self.cnn1 = nn.Conv2d(input_dim, 7, 2, stride=1, padding=1)
        self.cnn2 = nn.Conv2d(7, 4, 2, stride=1)
        #self.l1 = nn.Linear()
...
class Training():
    ...
    def train(self, iterator):
        ...
        for batch, input in enumerate(iterator, epoch, path):
            print(batch, input)
            self.optimizer.zero_grad()
            output = self.model(input)
...
def main():
    ...
    #data = np.load(".\..\..\cur_last_curTrue_msg.npy")
    data = np.load("cur_last_curTrue_msg.npy", allow_pickle=True)
